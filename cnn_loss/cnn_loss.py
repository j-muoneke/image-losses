...
class CNNLoss(nn.Module):
    ...
    def forward(self, recon: th.Tensor, real) -> th.Tensor:
        ...
        recon_early_features, recon_mid_features = self._resnet(recon, early_layer_only=mid_loss_w == 0)
        real_early_features, real_mid_features = self._resnet(real, early_layer_only=mid_loss_w == 0)
